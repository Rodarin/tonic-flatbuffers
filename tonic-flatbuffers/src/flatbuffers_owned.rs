...
use flatbuffers::{
    Follow, ForwardsUOffset, InvalidFlatbuffer, Verifiable, Verifier, VerifierOptions,
};
use std::ops::Deref;
...
/// ```
#[macro_export]
macro_rules! __flatbuffers_owned {
    ($struct_name:ident) => {
        $crate::flatbuffers_owned::paste! {
            impl $crate::flatbuffers_owned::RelaxedFollowTrait for $struct_name<'_> {
    ...
    ($($struct_name:ident),*) => {
        $(
            $crate::flatbuffers_owned::flatbuffers_owned!($struct_name);
        )*
    };
}

#[doc(inline)]
pub use __flatbuffers_owned as flatbuffers_owned;
